...
(function (window) {
    ...
    app.init = function () {
        ...
        for (var i = 0; i < app.count; i++) {
            ...
            var left = (i / (app.count / maxWidth));
            if (top === maxHeight) {
                m.classList.add('up');
            } else {
                m.classList.add('down');
            }
            m.style.transform = `translate(${left}vw, ${top}px)`;
    ...
    app.update = function (timestamp) {
        for (var i = 0; i < app.count; i++) {
            var m = movers[i];
            var pos = m.style.transform.match(/translate\(([0-9]*\.?[0-9]*)(.*),\s?([0-9]*\.?[0-9]*)(.*)\)/);
            var posx = parseInt(pos[1]),
                xExt = pos[2],
                posy = parseInt(pos[3]),
                yExt = pos[4];
            m.classList.contains('down') ? posy += distance : posy -= distance;
            if (posy < 0) posy = 0;
            if (posy > maxHeight) posy = maxHeight;
            m.style.transform = `translate(${posx}${xExt}, ${posy}${yExt})`;
            if (posy === 0) {
